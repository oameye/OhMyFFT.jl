"""
    mirror(v::AbstractVector) -> Vector
    mirror(ef::FFT)       -> Vector
...
This function is used internally in the `show` method for `FFT`.
...
"""
function finddomfreq(ef::FFT; n=5, t=0.1, window=length(ef)//50)
    absresp = abs.(ef.resp)
...
"""
function domfreq(ef::FFT; n=5, t=0.1, window=length(ef)//50)
...
of type `FFT`, as returned by `fft`.
...
```
julia> response_at(fft(rand(1000)), 0)
...
````
julia> response_at(fft(rand(1000)), 0.4415)
(freq = 0.441, resp = 0.003584422218085957 - 0.0025392417679877704im)
```

Response at multiple frequencies (long output supressed):
```
julia> response_at(fft(rand(1000)), [0, 0.1, 0.11, 0.111, 0.1111]);
```
````
"""
function response_at(ef::FFT, f::Real)
...
function response_at(ef::FFT, fs::AbstractVector{<:Real})
